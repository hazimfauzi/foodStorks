<?php
	...
	$result=mysqli_query($conn,"SELECT * FROM `order` WHERE order_type='1' AND done='0' AND rest_id='$rest_id' ORDER BY order_time AND order_date");
	
	//-------------------------------- For audio --------------------------------------
	$num_row=mysqli_num_rows($result);
	if($num_row>$_SESSION['count2']){
		echo "<audio controls autoplay hidden='true'><source src='chat.wav'></audio>";
		
	}$_SESSION['count2']=$num_row;
	//---------------------------------------------------------------------------------
        	...
        	<div class="panel panel-success">
				...
				<div class="panel-body">
					<table class="table table-hover" width="100%">
						<tr><th width="80%">Name</th><th>Quantity</th></tr><?php
...
var x = setInterval(function() {
  ...
  var distance = now - countDownDate<?php echo $row['order_id'] ?>;
  ...
  document.getElementById("demo<?php echo $row['order_id'] ?>").innerHTML = "TIME : " +  hours + "h "
  + minutes + "m " + seconds + "s ";
